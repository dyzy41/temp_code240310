...
def train_net(dataset_name):
    ...
    # t1_mean, t1_std = compute_mean_std(images_dir=os.path.join(data_root, 'A'))
    # t2_mean, t2_std = compute_mean_std(images_dir=os.path.join(data_root, 'B'))
    t1_mean, t1_std = [0.48338296, 0.48294251, 0.46055902], [0.1794798,  0.16979403, 0.17592412]
    t2_mean, t2_std = [0.48493343, 0.44443327, 0.38697436], [0.1426027,  0.13799887, 0.13799186]
    ...
    train_dataset = BasicDataset(t1_images_dir='{}/A'.format(data_root),
                                 t2_images_dir='{}/B'.format(data_root),
                                 labels_dir='{}/label'.format(data_root),
                                 train=True, **dataset_args)
    val_dataset = BasicDataset(t1_images_dir='{}/A'.format(data_root),
                               t2_images_dir='{}/B'.format(data_root),
                               labels_dir='{}/label'.format(data_root),
